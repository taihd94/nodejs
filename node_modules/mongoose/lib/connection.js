 ...
 * @param {String} [database] mongodb name
...
Connection.prototype.open = function(host, database, port, options, callback) {
  ...
  if (!database) {
    this.error(new Error('Missing mongodb name.'), callback);
    return new Promise.ES6(function(resolve, reject) {
      reject(err);
    });
  }
 ...
 *
 * The mongodb name and/or auth need only be included in one URI.
 ...
 * @param {String} [database] mongodb name if not included in `uris`
...
Connection.prototype.openSet = function(uris, database, options, callback) {
  ...
  if (!this.name) {
    var err = new Error('No mongodb name provided for replica set');
    this.error(err, callback);
    return new Promise.ES6(function(resolve, reject) {
      reject(err);
    });
