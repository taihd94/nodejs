 ...
 * @class
 * @param {string} databaseName The name of the mongodb this instance represents.
 * @param {(Server|ReplSet|Mongos)} topology The server topology for the mongodb.
 * @param {object} [options=null] Optional settings.
 * @param {string} [options.authSource=null] If the mongodb authentication is dependent on another databaseName.
 ...
 * @property {number} bufferMaxEntries Current bufferMaxEntries value for the mongodb
 * @property {string} databaseName The name of the mongodb this instance represents.
 ...
 * @param {string} [options.viewOn=null] The name of the source collection or view from which to create the view. The name is not the full namespace of the collection or view; i.e. does not include the mongodb name and implies the same mongodb as the view to create on MongoDB 3.4 or higher.
...
var listCollectionsTranforms = function(databaseName) {
  ...
  return {
    doc: function(doc) {
      ...
      // Remove mongodb name if available
      if(doc.name && index == 0) {
...
Db.prototype.listCollections = function(filter, options) {
  ...
  // No filter, filter by current mongodb
  if(filter == null) {
...
/**
 * Drop a collection from the mongodb, removing it permanently. New accesses will create a new collection.
 *
 * @method
...
/**
 * Drop a mongodb, removing it permanently from the server.
 *
 * @method
 * @param {Db~resultCallback} [callback] The results callback
 * @return {Promise} returns Promise if no callback passed
...
/**
 * Runs a command on the mongodb as admin.
 * @method
 * @param {object} command The command hash
 * @param {object} [options=null] Optional settings.
 * @param {(ReadPreference|string)} [options.readPreference=null] The preferred read preference (ReadPreference.PRIMARY, ReadPreference.PRIMARY_PREFERRED, ReadPreference.SECONDARY, ReadPreference.SECONDARY_PREFERRED, ReadPreference.NEAREST).
 ...
 * @param {boolean} [options.dropDups=false] A unique index cannot be created on a key that has pre-existing duplicate values. If you would like to create the index anyway, keeping the first document the mongodb indexes and deleting all subsequent documents that have duplicate value
 ...
 *
 * @method
 * @param {string} name The name of the mongodb we want to use.
 * @param {object} [options=null] Optional settings.
 ...
 * @method
 * @param {object} [options=null] Optional settings.
 * @param {string} [options.dbName=null] Logout against different mongodb than current.
 * @param {Db~resultCallback} [callback] The command result callback
 * @return {Promise} returns Promise if no callback passed
 */
Db.prototype.logout = function(options, callback) {
  ...
  // Establish the correct mongodb name
...
// Validate the mongodb name
var validateDatabaseName = function(databaseName) {
  if(typeof databaseName !== 'string') throw MongoError.create({message: "mongodb name must be a string", driver:true});
  if(databaseName.length === 0) throw MongoError.create({message: "mongodb name cannot be the empty string", driver:true});
  if(databaseName == '$external') return;

  var invalidChars = [" ", ".", "$", "/", "\\"];
  for(var i = 0; i < invalidChars.length; i++) {
    if(databaseName.indexOf(invalidChars[i]) != -1) throw MongoError.create({message: "mongodb names cannot contain the character '" + invalidChars[i] + "'", driver:true});
  }
}
